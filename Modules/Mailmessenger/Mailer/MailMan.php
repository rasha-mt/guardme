...
class MailMan
{
    ...
    public function prepare($htmlView, array $data = [], array $styles = [])
    {
        ...
        $css = file_get_contents(base_path('node_modules/semantic-ui/dist/semantic.min.css'));
        $css .= file_get_contents(base_path('node_modules/uikit/dist/css/uikit.min.css'));
        $css .= file_get_contents(public_path('/build/css/site.bundle.css'));
